...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    @Override
    public BpmTaskVo getFirstTodoTask(Long userId, String processInstanceId) {
        ...
        // 3. 转换返回
        BpmnModel bpmnModel = repositoryService.getBpmnModel(todoTask.getProcessDefinitionId());
        Map<Integer, BpmTaskVo.OperationButtonSetting> buttonsSetting = BpmnModelUtils.parseButtonsSetting(
                bpmnModel, todoTask.getTaskDefinitionKey());
        Boolean signEnable = BpmnModelUtils.parseSignEnable(bpmnModel, todoTask.getTaskDefinitionKey());
        Boolean reasonRequire = BpmnModelUtils.parseReasonRequire(bpmnModel, todoTask.getTaskDefinitionKey());

        // 4. 任务表单
        BpmForm taskForm = null;
        if (StrUtil.isNotBlank(todoTask.getFormKey())) {
            taskForm = bpmFormService.getById(NumberUtil.parseLong(todoTask.getFormKey(), null));
        }
    ...
    @Override
    public PageResult<HistoricTaskInstance> getTaskDonePage(Long userId, BpmTaskPageDto bpmTaskPageDto) {
    ...
    @Override
    public PageResult<HistoricTaskInstance> getTaskPage(Long userId, BpmTaskPageDto bpmTaskPageDto) {
        HistoricTaskInstanceQuery taskQuery = historyService.createHistoricTaskInstanceQuery()
                .includeTaskLocalVariables()
    ...
    private Task validateTask(Long userId, String taskId) {
        ...
        if (StrUtil.isNotBlank(task.getAssignee())
                && ObjectUtil.notEqual(userId, NumberUtil.parseLong(task.getAssignee(), null))) {
            throw ServerException.of(BpmResponseStatus.TASK_OPERATE_FAIL_ASSIGN_NOT_SELF);
        }
        return task;
    }
    ...
    public <T extends TaskInfo> List<T> getAllChildrenTaskListByParentTaskId(String parentTaskId, List<T> tasks) {
        ...
        Map<String, List<T>> parentTaskMap = tasks.stream()
                .filter(task -> StrUtil.isNotEmpty(task.getParentTaskId()))
                .collect(Collectors.groupingBy(TaskInfo::getParentTaskId));
        ...
        for (int i = 0; i < Short.MAX_VALUE; i++) {
            ...
            List<T> childTaskList = tasks.stream().filter(task -> StrUtil.equals(task.getParentTaskId(), taskId)).toList();
            // 2.2 如果非空，则添加到 stack 进一步递归
            if (CollUtil.isNotEmpty(childTaskList)) {
                stack.addAll(childTaskList.stream().map(TaskInfo::getId).toList());
    ...
    public void approveTask(Long userId, @Valid BpmTaskApproveDto reqVO) {
        // 1.1 校验任务存在
        Task task = validateTask(userId, reqVO.getId());
        // 1.2 校验流程实例存在
        ProcessInstance instance = bpmProcessInstanceService.getProcessInstance(task.getProcessInstanceId());
        if (instance == null) {
            throw ServerException.of(BpmResponseStatus.PROCESS_INSTANCE_NOT_EXISTS);
        }
        // 1.3 校验签名
        BpmnModel bpmnModel = repositoryService.getBpmnModel(task.getProcessDefinitionId());
        Boolean signEnable = BpmnModelUtils.parseSignEnable(bpmnModel, task.getTaskDefinitionKey());
        if (signEnable && StrUtil.isEmpty(reqVO.getSignPicUrl())) {
            throw ServerException.of(BpmResponseStatus.TASK_SIGNATURE_NOT_EXISTS);
        }
        // 1.4 校验审批意见
        Boolean reasonRequire = BpmnModelUtils.parseReasonRequire(bpmnModel, task.getTaskDefinitionKey());
        if (reasonRequire && StrUtil.isEmpty(reqVO.getReason())) {
            throw ServerException.of(BpmResponseStatus.TASK_REASON_REQUIRE);
        }
        ...
        // 情况二：审批有【后】加签的任务
        if (BpmTaskSignTypeEnum.AFTER.getValue().equals(task.getScopeType())) {
        ...
        // 2.2 添加评论
        taskService.addComment(task.getId(), task.getProcessInstanceId(), BpmCommentTypeEnum.APPROVE.getValue(),
    ...
    private void handleParentTaskIfSign(String parentTaskId) {
        ...
        // 3.1 情况一：处理向【向前】加签
        if (BpmTaskSignTypeEnum.BEFORE.getValue().equals(scopeType)) {
            // 3.1.1 owner 重新赋值给父任务的 assignee，这样它就可以被审批
            taskService.resolveTask(parentTaskId);
            // 3.1.2 更新流程任务 status
            updateTaskStatus(parentTaskId, BpmTaskStatusEnum.RUNNING.getStatus());
            // 3.2 情况二：处理向【向后】加签
        } else if (BpmTaskSignTypeEnum.AFTER.getValue().equals(scopeType)) {
    ...
    private void approveDelegateTask(BpmTaskApproveDto reqVO, Task task) {
        // 1. 添加审批意见
        BpmUserVo currentUser = bpmUserApi.getUser(LoginUserContext.currentUserId());
        BpmUserVo ownerUser = bpmUserApi.getUser(NumberUtil.parseLong(task.getOwner(), null)); // 发起委托的用户
        Assert.notNull(ownerUser, "委派任务找不到原审批人，需要检查数据");
        taskService.addComment(reqVO.getId(), task.getProcessInstanceId(), BpmCommentTypeEnum.DELEGATE_END.getValue(),
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectDto reqVO) {
        ...
        // 2.2 添加流程评论
        taskService.addComment(task.getId(), task.getProcessInstanceId(), BpmCommentTypeEnum.REJECT.getValue(),
        ...
        if (task.getParentTaskId() != null) {
            ...
            taskService.addComment(rootParentId, task.getProcessInstanceId(), BpmCommentTypeEnum.REJECT.getValue(),
                    BpmCommentTypeEnum.REJECT.formatComment("加签任务不通过"));
        }
        ...
        // 3.2 情况二：直接结束，审批不通过
        bpmProcessInstanceService.updateProcessInstanceReject(instance, reqVO.getReason()); // 标记不通过
    ...
    public void returnTask(Long userId, BpmTaskReturnDto reqVO) {
        ...
        Task task = validateTask(userId, reqVO.getId());
        if (task.isSuspended()) {
            throw ServerException.of(BpmResponseStatus.TASK_IS_PENDING);
        }
    ...
    private FlowElement validateTargetTaskCanReturn(String sourceKey, String targetKey, String processDefinitionId) {
        ...
        if (target == null) {
            throw ServerException.of(BpmResponseStatus.TASK_TARGET_NODE_NOT_EXISTS);
        }

        // 2.2 只有串行可到达的节点，才可以退回。类似非串行、子流程无法退回
        if (!BpmnModelUtils.isSequentialReachable(source, target, null)) {
            throw ServerException.of(BpmResponseStatus.TASK_RETURN_FAIL_SOURCE_TARGET_ERROR);
        }
        return target;
    }
     ...
     * @param reqVO         前端参数封装
     */
    public void returnTask(Long userId, Task currentTask, FlowElement targetElement, BpmTaskReturnDto reqVO) {
        ...
        taskList.forEach(task -> {
            ...
            if (isAssignUserTask(userId, task)) { // 情况一：自己的任务，进行 RETURN 标记
                // 2.1.1 添加评论
                taskService.addComment(task.getId(), currentTask.getProcessInstanceId(), BpmCommentTypeEnum.RETURN.getValue(),
    ...
    public void delegateTask(Long userId, BpmTaskDelegateDto reqVO) {
        ...
        if (task.getAssignee().equals(reqVO.getDelegateUserId().toString())) { // 校验当前审批人和被委派人不是同一人
            throw ServerException.of(BpmResponseStatus.TASK_DELEGATE_FAIL_USER_REPEAT);
        }
        // 1.2 校验目标用户存在
        BpmUserVo delegateUser = bpmUserApi.getUser(reqVO.getDelegateUserId());
        if (delegateUser == null) {
            throw ServerException.of(BpmResponseStatus.TASK_DELEGATE_FAIL_USER_NOT_EXISTS);
        }

        // 2. 添加委托意见
        BpmUserVo currentUser = bpmUserApi.getUser(userId);
        taskService.addComment(taskId, task.getProcessInstanceId(), BpmCommentTypeEnum.DELEGATE_START.getValue(),
    ...
    public void transferTask(Long userId, BpmTaskTransferDto reqVO) {
        ...
        if (task.getAssignee().equals(reqVO.getAssigneeUserId().toString())) { // 校验当前审批人和被转派人不是同一人
            throw ServerException.of(BpmResponseStatus.TASK_TRANSFER_FAIL_USER_REPEAT);
        }
        // 1.2 校验目标用户存在
        BpmUserVo assigneeUser = bpmUserApi.getUser(reqVO.getAssigneeUserId());
        if (assigneeUser == null) {
            throw ServerException.of(BpmResponseStatus.TASK_TRANSFER_FAIL_USER_NOT_EXISTS);
        }

        // 2. 添加委托意见
        BpmUserVo currentUser = bpmUserApi.getUser(userId);
        taskService.addComment(taskId, task.getProcessInstanceId(), BpmCommentTypeEnum.TRANSFER.getValue(),
    ...
    public void moveTaskToEnd(String processInstanceId, String reason) {
        ...
        List<String> activityIds = CollUtil.newArrayList(taskList.stream().map(Task::getTaskDefinitionKey).filter(StrUtil::isNotEmpty).collect(Collectors.toSet()));
        ...
        // 目前发现并行分支情况下，会存在这个情况，后续看看有没更好的方案；
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void createSignTask(Long userId, BpmTaskSignDto reqVO) {
        // 1. 获取和校验任务
        TaskEntityImpl taskEntity = validateTaskCanCreateSign(userId, reqVO);
        List<BpmUserVo> userList = bpmUserApi.getUsers(List.copyOf(reqVO.getUserIds()));
        if (CollUtil.isEmpty(userList)) {
            throw ServerException.of(BpmResponseStatus.TASK_SIGN_CREATE_USER_NOT_EXIST);
        }
        ...
        if (reqVO.getType().equals(BpmTaskSignTypeEnum.BEFORE.getValue())) {
        ...
        // 3. 创建加签任务
        createSignTaskList(reqVO.getUserIds().stream().map(String::valueOf).toList(), taskEntity);

        // 4. 记录加签的评论到 task 任务
        BpmUserVo currentUser = bpmUserApi.getUser(userId);
        String comment = StrUtil.format(BpmCommentTypeEnum.ADD_SIGN.getComment(),
                currentUser.getNickname(), BpmTaskSignTypeEnum.of(reqVO.getType()),
                String.join(",", userList.stream().map(BpmUserVo::getNickname).toList()), reqVO.getReason());
        taskService.addComment(reqVO.getId(), taskEntity.getProcessInstanceId(), BpmCommentTypeEnum.ADD_SIGN.getValue(), comment);
    }
     ...
     * @return 当前任务
     */
    private TaskEntityImpl validateTaskCanCreateSign(Long userId, BpmTaskSignDto reqVO) {
        ...
        if (taskEntity.getScopeType() != null
            ...
            throw ServerException.of(BpmResponseStatus.TASK_SIGN_CREATE_TYPE_ERROR,
                    BpmTaskSignTypeEnum.of(taskEntity.getScopeType()), BpmTaskSignTypeEnum.of(reqVO.getType()));
        }
        ...
        List<Long> currentAssigneeList = taskList.stream().flatMap(task -> // 需要考虑 owner 的情况，因为向后加签时，它暂时没 assignee 而是 owner
                Stream.of(NumberUtil.parseLong(task.getAssignee(), null), NumberUtil.parseLong(task.getOwner(), null))).toList();
        if (CollUtil.containsAny(currentAssigneeList, reqVO.getUserIds())) {
            List<BpmUserVo> userList = bpmUserApi.getUsers(CollUtil.intersection(currentAssigneeList, reqVO.getUserIds()).stream().toList());
            throw ServerException.of(BpmResponseStatus.TASK_SIGN_CREATE_USER_REPEAT, String.join(",", userList.stream().map(BpmUserVo::getNickname).toList()));
        }
        return taskEntity;
    }
    ...
    private void createSignTask(TaskEntityImpl parentTask, String assignee) {
        ...
        // 2.1 向前加签，设置审批人
        if (BpmTaskSignTypeEnum.BEFORE.getValue().equals(parentTask.getScopeType())) {
        ...
        if (BpmTaskSignTypeEnum.AFTER.getValue().equals(parentTask.getScopeType())) {
    ...
    public void deleteSignTask(Long userId, BpmTaskSignDeleteDto reqVO) {
        ...
        // 1.2 校验取消人存在
        BpmUserVo cancelUser = null;
        if (StrUtil.isNotBlank(task.getAssignee())) {
            cancelUser = bpmUserApi.getUser(NumberUtil.parseLong(task.getAssignee(), null));
        }
        if (cancelUser == null && StrUtil.isNotBlank(task.getOwner())) {
            cancelUser = bpmUserApi.getUser(NumberUtil.parseLong(task.getOwner(), null));
        }
        ...
        // 2.3 删除任务和所有子任务
        taskService.deleteTasks(childTaskList.stream().map(Task::getId).toList());

        // 3. 记录日志到父任务中。先记录日志是因为，通过 handleParentTask 方法之后，任务可能被完成了，并且不存在了，会报异常，所以先记录
        BpmUserVo user = bpmUserApi.getUser(userId);
        taskService.addComment(task.getParentTaskId(), task.getProcessInstanceId(), BpmCommentTypeEnum.SUB_SIGN.getValue(),
    ...
    private Task validateTaskCanSignDelete(String id) {
        Task task = validateTaskExist(id);
        if (task.getParentTaskId() == null) {
            throw ServerException.of(BpmResponseStatus.TASK_SIGN_DELETE_NO_PARENT);
        }
        Task parentTask = getTask(task.getParentTaskId());
        if (parentTask == null) {
            throw ServerException.of(BpmResponseStatus.TASK_SIGN_DELETE_NO_PARENT);
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(int transactionStatus) {
                ...
                if (ObjectUtil.equal(approveType, BpmUserTaskApproveTypeEnum.USER.getValue())) {
                    // 如果有审批人、或者拥有人，则说明不满足情况一，不自动通过、不自动拒绝
                    if (!ObjectUtil.isAllEmpty(task.getAssignee(), task.getOwner())) {
                        return;
                    }
                    if (ObjectUtil.equal(assignEmptyHandlerType, BpmUserTaskAssignEmptyHandlerTypeEnum.APPROVE.getValue())) {
                        getSelf().approveTask(null, new BpmTaskApproveDto()
                                .setId(task.getId()).setReason(BpmReasonEnum.ASSIGN_EMPTY_APPROVE.getReason()));
                    } else if (ObjectUtil.equal(assignEmptyHandlerType, BpmUserTaskAssignEmptyHandlerTypeEnum.REJECT.getValue())) {
                    ...
                    if (ObjectUtil.equal(approveType, BpmUserTaskApproveTypeEnum.AUTO_APPROVE.getValue())) {
                        getSelf().approveTask(null, new BpmTaskApproveDto()
                                .setId(task.getId()).setReason(BpmReasonEnum.APPROVE_TYPE_AUTO_APPROVE.getReason()));
                    } else if (ObjectUtil.equal(approveType, BpmUserTaskApproveTypeEnum.AUTO_REJECT.getValue())) {
                        getSelf().rejectTask(null, new BpmTaskRejectDto()
    ...
    public void processTaskAssigned(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(int transactionStatus) {
                ...
                // 自动去重，通过自动审批的方式 驳回的情况得考虑一下；驳回后，又自动审批么？
                BpmProcessDefinitionInfo processDefinitionInfo = bpmProcessDefinitionInfoService.getProcessDefinitionInfo(task.getProcessDefinitionId());
                ...
                if (processDefinitionInfo.getAutoApprovalType() != null) {
                    HistoricTaskInstanceQuery sameAssigneeQuery = historyService.createHistoricTaskInstanceQuery()
                            ...
                            .finished();
                    if (BpmAutoApproveTypeEnum.APPROVE_ALL.getValue().equals(processDefinitionInfo.getAutoApprovalType())
                        && sameAssigneeQuery.count() > 0) {
                        getSelf().approveTask(Long.valueOf(task.getAssignee()), new BpmTaskApproveDto().setId(task.getId())
                                .setReason(BpmAutoApproveTypeEnum.APPROVE_ALL.getDesc()));
                        return;
                    }
                    if (BpmAutoApproveTypeEnum.APPROVE_SEQUENT.getValue().equals(processDefinitionInfo.getAutoApprovalType())) {
                        BpmnModel bpmnModel = repositoryService.getBpmnModel(processInstance.getProcessDefinitionId());
                        if (bpmnModel == null) {
                            log.error("[processTaskAssigned][taskId({}) 没有找到流程模型({})]", task.getId(), task.getProcessDefinitionId());
                            return;
                        }
                        List<String> sourceTaskIds = BpmnModelUtils.getElementIncomingFlows( // 获取所有上一个节点
                                BpmnModelUtils.getFlowElementById(bpmnModel, task.getTaskDefinitionKey())).stream().map(SequenceFlow::getSourceRef).toList();
                        if (sameAssigneeQuery.taskDefinitionKeys(sourceTaskIds).count() > 0) {
                            getSelf().approveTask(Long.valueOf(task.getAssignee()), new BpmTaskApproveDto().setId(task.getId())
                                    .setReason(BpmAutoApproveTypeEnum.APPROVE_SEQUENT.getDesc()));
                            return;
                        }
                    }
                }
                ...
                if (StrUtil.equals(task.getAssignee(), processInstance.getStartUserId())) {
                    ...
                    if (ObjUtil.notEqual(returnTaskFlag, Boolean.TRUE)) {
                        ...
                        // 情况一：自动跳过
                        if (Objects.equals(assignStartUserHandlerType,
                                BpmUserTaskAssignStartUserHandlerTypeEnum.SKIP.getValue())) {
                            getSelf().approveTask(Long.valueOf(task.getAssignee()), new BpmTaskApproveDto().setId(task.getId())
    ...
    public void processTaskTimeout(String processInstanceId, String taskDefineKey, Integer handlerType) {
        ...
        taskList.forEach(task -> FlowableUtils.execute(task.getTenantId(), () -> {
            // 情况一：自动提醒
            if (Objects.equals(handlerType, BpmUserTaskTimeoutHandlerTypeEnum.REMINDER.getValue())) {
                messageService.sendMessageWhenTaskTimeout(new BpmMessageSendWhenTaskTimeoutDto()
            ...
            // 情况二：自动同意
            if (Objects.equals(handlerType, BpmUserTaskTimeoutHandlerTypeEnum.APPROVE.getValue())) {
            ...
            // 情况三：自动拒绝
            if (Objects.equals(handlerType, BpmUserTaskTimeoutHandlerTypeEnum.REJECT.getValue())) {
