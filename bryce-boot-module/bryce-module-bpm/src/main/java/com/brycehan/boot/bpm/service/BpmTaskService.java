...
public interface BpmTaskService {
     ...
     *
     * @param userId    用户编号
     * @param bpmTaskPageDto 分页请求
     * @return 流程任务分页
     */
    PageResult<Task> getTaskTodoPage(Long userId, BpmTaskPageDto bpmTaskPageDto);
     ...
     * @return 待办任务
     */
    BpmTaskVo getFirstTodoTask(Long userId, String processInstanceId);
     ...
     * @return 流程任务分页
     */
    PageResult<HistoricTaskInstance> getTaskDonePage(Long userId, BpmTaskPageDto bpmTaskPageDto);
     ...
     * @return 流程任务分页
     */
    PageResult<HistoricTaskInstance> getTaskPage(Long userId, BpmTaskPageDto bpmTaskPageDto);
    ...
    default Map<String, List<Task>> getTaskMapByProcessInstanceIds(List<String> processInstanceIds) {
        ...
        List<Task> tasks = getTasksByProcessInstanceIds(processInstanceIds);
        return tasks.stream().collect(Collectors.groupingBy(Task::getProcessInstanceId, Collectors.mapping(Function.identity(), Collectors.toList())));
    }
     ...
     */
    List<Task> getRunningTaskListByProcessInstanceId(String processInstanceId, Boolean assigned, String taskDefineKey);
     ...
     *
     * @param userId 用户编号
     * @param bpmTaskApproveDto  通过请求
     */
    void approveTask(Long userId, BpmTaskApproveDto bpmTaskApproveDto);

    /**
     * 不通过任务
     *
     * @param userId 用户编号
     * @param bpmTaskRejectDto  不通过请求
     */
    void rejectTask(Long userId, BpmTaskRejectDto bpmTaskRejectDto);

    /**
     * 将流程任务分配给指定用户
     *
     * @param userId 用户编号
     * @param bpmTaskTransferDto  分配请求
     */
    void transferTask(Long userId, BpmTaskTransferDto bpmTaskTransferDto);
     ...
     *
     * @param userId 用户编号
     * @param bpmTaskReturnDto  退回的任务key和当前所在的任务ID
     */
    void returnTask(Long userId, BpmTaskReturnDto bpmTaskReturnDto);

    /**
     * 将指定任务委派给其他人处理，等接收人处理后再回到原审批人手中审批
     *
     * @param userId 用户编号
     * @param bpmTaskDelegateDto  被委派人和被委派的任务编号理由参数
     */
    void delegateTask(Long userId, BpmTaskDelegateDto bpmTaskDelegateDto);

    /**
     * 任务加签
     *
     * @param userId 被加签的用户和任务 ID，加签类型
     * @param bpmTaskSignDto  当前用户 ID
     */
    void createSignTask(Long userId, BpmTaskSignDto bpmTaskSignDto);

    /**
     * 任务减签
     *
     * @param userId 当前用户ID
     * @param bpmTaskSignDeleteDto  被减签的任务 ID，理由
     */
    void deleteSignTask(Long userId, BpmTaskSignDeleteDto bpmTaskSignDeleteDto);

    /**
     * 抄送任务
     *
     * @param userId 用户编号
     * @param bpmTaskCopyDto  通过请求
     */
    void copyTask(Long userId, BpmTaskCopyDto bpmTaskCopyDto);
