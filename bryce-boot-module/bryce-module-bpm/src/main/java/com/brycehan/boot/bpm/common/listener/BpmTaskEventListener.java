...
import com.brycehan.boot.bpm.common.type.BpmBoundaryEventTypeEnum;
...
//            .add(FlowableEngineEventType.TASK_COMPLETED) // 由于审批通过时，已经记录了 task 的 status 为通过，所以不需要监听了。
    ...
    protected void timerFired(FlowableEngineEntityEvent event) {
        ...
        BpmBoundaryEventTypeEnum bpmTimerBoundaryEventType = BpmBoundaryEventTypeEnum.of(NumberUtil.parseInt(boundaryEventType, null));
        ...
        if (ObjectUtil.equal(bpmTimerBoundaryEventType, BpmBoundaryEventTypeEnum.USER_TASK_TIMEOUT)) {
            ...
            taskService.processTaskTimeout(event.getProcessInstanceId(), taskKey, NumberUtil.parseInt(timeoutHandlerType, null));
            // 2.2 延迟器超时处理
